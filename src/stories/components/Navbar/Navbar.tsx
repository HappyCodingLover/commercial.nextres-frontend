...
export const Navbar = ({
  ...
  return (
    <nav className="bg-white border-gray-200 px-2 sm:px-4 py-2.5 rounded dark:bg-gray-800" {...props}>
      <div className="container flex flex-wrap justify-between items-center mx-auto">
        <a href="#" className="flex items-center">
          {imgLogo && <img src={imgLogo} className="mr-3 h-6 sm:h-9" alt={`${title} Logo`} />}
        ...
        <div className="hidden w-full md:block md:w-auto" id="mobile-menu">
          <ul className="flex flex-col mt-4 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium">
            {navigations.map((nav) => {
              const isActive = activeNav.toLowerCase() === nav.toLowerCase()
              return (
                <li>
                  <a
                    href="#"
